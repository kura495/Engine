...
uint32_t TextureManager::LoadTexture(const std::string& filePath)
{
	...
	textures_.at(index).textureSrvHandleCPU = sRVManager_->GetDescriptorHandle(index).CPU;
	textures_.at(index).textureSrvHandleGPU = sRVManager_->GetDescriptorHandle(index).GPU;
